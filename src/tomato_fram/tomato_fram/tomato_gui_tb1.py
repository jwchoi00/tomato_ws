...
class VideoPlayer(QMainWindow, form_class, Node):
    def __init__(self):
        ...
        self.detect_subscriber = self.create_subscription(
            TomatoDetected, '/tb1/tomato/detected', self.tomato_detected_callback, 50
        )
        self.image_subscriber = self.create_subscription(
            Image, '/tb1/tomato/image', self.image_callback, 50
        )
        self.action_client = ActionClient(self, NavigateToPose, '/tb1/navigate_to_pose')
        # Publisher for sending position to the transport robot
        self.position_publisher = self.create_publisher(PoseStamped, '/tb1/odom', 10)

        # Service client to request transport robot to go home
        self.transport_go_home_client = self.create_client(TransportRobot, '/tb2/go_home')
